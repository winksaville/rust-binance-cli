...
pub fn arg_matches() -> Result<ArgMatches, Box<dyn Error>> {
    ...
    let matches = Command::new(APP_NAME.as_str())
        ...
        .arg(api_key_arg)
        .arg(secret_key_arg)
        .arg(order_log_path_arg)
        .arg(default_quote_asset_arg)
        .arg(test_arg)
        .arg(no_test_arg)
        .arg(verbose_arg)
        .arg(no_verbose_arg)
        .arg(progress_info_arg)
        .arg(no_progress_info_arg)
        .arg(throttle_rate_ms_arg)
        .arg(confirmation_required_arg)
        .arg(no_confirmation_required_arg)
        .arg(scheme_arg)
        .arg(domain_arg)
