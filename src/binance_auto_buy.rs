...
mod test {
    ...
    fn test_config_auto_buy_all() {
        ...
        // XYZ is odd as nothing will be sold since BuyRec.min default is MAX so quote_asset is ignored
        assert_eq!(
            config.buy.get("XYZ").unwrap(),
            &BuyRec {
                name: "XYZ".to_string(),
                percent: Decimal::MAX,
